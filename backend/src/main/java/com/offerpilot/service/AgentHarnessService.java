...
public class AgentHarnessService {
    ...
    public AgentHarnessService(OfferRepository offers,CityDataController cities,KnowledgeService knowledge,IncomeCalculator calculator,
      ...
      @Value("${spring.ai.openai.chat.options.model:qwen-plus}") String qwenModel,@Value("${offerpilot.ai.enabled:false}") boolean enabled){
        ...
        SimpleClientHttpRequestFactory http=new SimpleClientHttpRequestFactory();http.setConnectTimeout(10_000);http.setReadTimeout(90_000);
        this.qwen=RestClient.builder().baseUrl(qwenBase).requestFactory(http).build();this.aiEnabled=enabled&&chatClient!=null&&!qwenKey.isBlank();
    }
    ...
    private String executeWeb(RunContext run,String query){checkLimit(run);long started=System.nanoTime();String key="offerpilot:web:"+Integer.toHexString(query.trim().toLowerCase(Locale.ROOT).hashCode());boolean hit=false;String out=null,status="COMPLETED";try{out=redis.opsForValue().get(key);hit=out!=null;}catch(RuntimeException ignored){}try{if(out==null){out=searchWeb(query);try{redis.opsForValue().set(key,out,Duration.ofMinutes(15));}catch(RuntimeException ignored){}}}catch(Exception e){status="FAILED";out="ERROR: "+safe(e);}record(run,"realtime_web_search",query,out,status,elapsed(started),hit);return out;}
    ...
    private long elapsed(long s){return(System.nanoTime()-s)/1_000_000;}private String safe(Throwable e){StringBuilder b=new StringBuilder();Throwable current=e;for(int i=0;i<4&&current!=null;i++,current=current.getCause()){if(i>0)b.append(" <- ");b.append(current.getClass().getSimpleName()).append(": ").append(current.getMessage()==null?"无详细信息":current.getMessage());}return trim(b.toString(),600);}private String trim(String s,int n){return s==null?"":s.substring(0,Math.min(s.length(),n));}
